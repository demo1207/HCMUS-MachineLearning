...
function compute_average_intensity(x, y)
    ...
    #TODO compute average intensity for each label
	num_labels = 10  # Assuming there are 10 different digits (0 to 9)
    sum_pixel_values = zeros(Float64, num_labels)
    sample_counts = zeros(Int, num_labels)
    for i in 1:size(x, 1)
        label = y[i] + 1 
        sum_pixel_values[label] += sum(x[i, :])
        sample_counts[label] += 1
    end
    mean_ = sum_pixel_values ./ sample_counts
...
4. **Numerical Stability:** Some algorithms, like SVMs or neural networks, may be numerically unstable if the input features have large differences in scale. Normalization can help mitigate this issue.
"""
...
function normalize(x, mean_=nothing, std_=nothing)
   if mean_ == nothing && std_ == nothing
	...
	return normalized_x, mean_, std_ #return `normalized_train_x` calculated by using mean_ and std_ (both of them are passed and not null)
end
...
function sigmoid_activation(x)
    ...
    #return?
	activation_value = 1.0 ./ (1.0 .+ exp.(-x))
...
function compute_gradient(error, train_x)
    ...
    """
    return transpose(train_x) * error / size(train_x, 1)
end
...
begin
	...
	normalized_test_x = normalize(test_x_new, mean_, std_)[1]
	add_one_test_x = hcat(ones(size(test_x_new)[1],), normalized_test_x)
	size(add_one_test_x)
end
...
**Comments:**
Accuracy Concerns: Overall accuracy is quite low, indicating potential model enhancements.
