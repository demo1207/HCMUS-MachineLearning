...
function entropy_of_one_division(division)
	...
	for sample in n_classes
		push!(counts, count(i->(i==sample), division))
	end
	entropy_value = entropy(counts, n_samples)
		

    return entropy_value, n_samples
...
function get_entropy(y_predict, y)
	...
	# println("TRUE:", entropy_true, n_true)
	# println("FALSE:", entropy_false, n_false)
    # overall entropy
	 #TODO s=?
	p_true = n_true / n
    p_false = n_false / n
...
function maxHypo(hist)
    #find the hypothesis with maximum probability from dictionary hist
    #TODO
	max_prob = maximum(values(hist))
	max_hypo = 0
	for (key, val) in hist
		if val == max_prob
			max_hypo = key
...
function likelihood(_mean=nothing, _std=nothing ,data=nothing, hypo=nothing)
	...
	for i in 1:length(data)
        res *= pdf(Normal(mean[i], std[i]), data[i])
    end
...
# ╔═╡ bced5e85-3c69-47b4-8211-af243441abd4
# Custom standard deviation function
function custom_std(data, mean_val)
    return sqrt(sum((data .- mean_val) .^ 2) / length(data))
end

# ╔═╡ bb344341-9902-41e8-b6dd-052019466844
...
function gfit(X, y, _std=nothing, _mean=nothing, _hist=nothing)
    ...
    for hypo in Set(y)
		...
		mean[hypo] = [custom_mean(rows[:, col]) for col in 1:size(rows, 2)]
        std[hypo] = [custom_std(rows[:, col], mean[hypo][col]) for col in 1:size(rows, 2)]
...
begin
	...
	for i ∈ [0, 1, 2]
		# Calculate true positives, false positives, false negatives, and true negatives
	    tp, fp, tn, fn = tpfptnfn_cal(y_test, _pred, i)
