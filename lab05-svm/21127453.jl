...
# ╔═╡ cd23efe5-1b72-4c38-a2a1-500e2e6cea77
using ScikitLearn
...
student = (name = "Hoàng Anh Trà", id = "21127453")
...
function pla(pos_data::Matrix{Float64}, neg_data::Matrix{Float64}, 
	...
	# START YOUR CODE
    n = size(pos_data, 2)
    data = vcat(hcat(pos_data, neg_data), ones(n * 2)')'
    labels = vcat(ones(n), -ones(n))
    θ = zeros(size(data, 2))
...
# ╔═╡ 9aa177c4-5858-439a-971b-c6d411a5f6af
...
Denote $\theta^{(k)}$ as the parameter vector after $k$ updates, and $\theta^*$ as the optimal parameter vector. The PLA update rule is given by:

$\begin{cases}
\theta^{(k+1)} \leftarrow \theta^{(k)} + y_t\mathbf{x}_t \text{ if } y_t(\theta^{(k+1)})^\top\mathbf{x}_t < 0 \\
\text{The parameters remain unchanged otherwise}
\end{cases}$

When an update is performed ($y_t(\theta^{(k+1)})^\top \mathbf{x}_t < 0$), the update rule is:

$\theta^{(k+1)} = \theta^{(k)} + y_t \mathbf{x}_t$

Now, let's calculate the inner product $(\theta^*)^\top \theta^{(k+1)}$:

$\begin{align*}
(\theta^*)^\top \theta^{(k+1)} &= (\theta^*)^\top (\theta^{(k)} + y_t \mathbf{x}_t) \\
&= (\theta^*)^\top \theta^{(k)} + (\theta^*)^\top (y_t \mathbf{x}_t)
\end{align*}$

Considering the update condition $y_t(\theta^{(k+1)})^\top\mathbf{x}_t < 0$ and $y_t(\theta^{(k)})^\top\mathbf{x}_t < 0$, we have $(\theta^*)^\top \mathbf{x}_t < 0$. Now, consider the term $(\theta^*)^\top (y_t \mathbf{x}_t)$:

$(\theta^*)^\top (y_t \mathbf{x}_t) = y_t (\theta^*)^\top \mathbf{x}_t < 0$

Therefore, we can rewrite the inner product as:

$(\theta^*)^\top \theta^{(k+1)} = (\theta^*)^\top \theta^{(k)} + (\theta^*)^\top (y_t \mathbf{x}_t) < (\theta^*)^\top \theta^{(k)}$
...
Denote $\theta^{(k)}$ as the parameter vector after $k$ updates in the Perceptron Learning Algorithm (PLA). The squared norm $||\theta^{(k)}||^2$ is given by:

$||\theta^{(k)}||^2 = (\theta^{(k)})^\top \theta^{(k)}$

Now, consider the PLA update rule:
...
Now, let's calculate the squared norm $||\theta^{(k+1)}||^2$:

$\begin{align*}
||\theta^{(k+1)}||^2 &= (\theta^{(k+1)})^\top \theta^{(k+1)} \\
&= (\theta^{(k)} + y_t \mathbf{x}_t)^\top (\theta^{(k)} + y_t \mathbf{x}_t) \\
&= (\theta^{(k)})^\top \theta^{(k)} + 2y_t(\theta^{(k)})^\top \mathbf{x}_t + y_t^2 ||\mathbf{x}_t||^2
\end{align*}$

Now, consider the update condition $y_t(\theta^{(k+1)})^\top\mathbf{x}_t < 0$ and $y_t(\theta^{(k)})^\top\mathbf{x}_t < 0$. We have $y_t(\theta^{(k)})^\top\mathbf{x}_t < 0$ since an update is performed. Therefore, $2y_t(\theta^{(k)})^\top \mathbf{x}_t < 0$.

Now, we can rewrite the squared norm $||\theta^{(k+1)}||^2$ as:

$||\theta^{(k+1)}||^2 = (\theta^{(k)})^\top \theta^{(k)} + 2y_t(\theta^{(k)})^\top \mathbf{x}_t + y_t^2 ||\mathbf{x}_t||^2 < ||\theta^{(k)}||^2 + y_t^2 ||\mathbf{x}_t||^2$

Therefore, we have shown that $||\theta^{(k+1)}||^2$ increases at most quadratically in the number of updates $k$.
...
Let $\mathbf{x}_0$ be a point on the hyperplane defined by $\mathbf{\theta}^*$. The distance from a point $\mathbf{x}$ to the hyperplane is given by:

$d(\mathbf{x}, \mathbf{\theta}) = \frac{|\mathbf{\theta}^\top \mathbf{x} + b|}{||\mathbf{\theta}||}$

Where $\mathbf{\theta}$ is the normal vector to the hyperplane, and $b$ is the bias term.

Now, consider two hyperplanes defined by weight vectors $\mathbf{\theta}$ and $\mathbf{\theta}^*$. The distance between these two hyperplanes is the distance between a point on one hyperplane and the other hyperplane. Let $\mathbf{x}_0$ be a point on the hyperplane defined by $\mathbf{\theta}^*$. The distance between the two hyperplanes is then:

$d = \frac{|\mathbf{\theta}^\top \mathbf{x}_0 + b|}{||\mathbf{\theta}||}$

We aim to show that this distance is equal to $\frac{2}{||\mathbf{\theta}||}$.

Now, consider the point $\mathbf{x}_1 = \mathbf{x}_0 + \frac{\mathbf{\theta}}{||\mathbf{\theta}||}$. This point lies on the hyperplane defined by $\mathbf{\theta}$. Calculate the distance between $\mathbf{x}_1$ and the hyperplane defined by $\mathbf{\theta}^*$:

$d' = \frac{|\mathbf{\theta}^{*^\top} \mathbf{x}_1 + b^*|}{||\mathbf{\theta}^*||}$

Since $\mathbf{x}_1$ lies on the hyperplane defined by $\mathbf{\theta}$, we have $\mathbf{\theta}^\top \mathbf{x}_1 + b = 0$. Substitute this into the expression for $d'$:

$d' = \frac{|\mathbf{\theta}^{*^\top} \mathbf{x}_1 + b^*|}{||\mathbf{\theta}^*||} = \frac{|b^*|}{||\mathbf{\theta}^*||}$

Now, we can relate $d$ and $d'$:

$d = \frac{|\mathbf{\theta}^\top \mathbf{x}_0 + b|}{||\mathbf{\theta}||} = \frac{|b|}{||\mathbf{\theta}||}$

To show that $d = \frac{2}{||\mathbf{\theta}||}$, we need to show that $|b| = \frac{1}{2}$. This implies that $\mathbf{x}_0$ is a point on the hyperplane defined by $\frac{\mathbf{\theta}}{2}$.

Therefore, the distance between the hyperplanes is indeed $\frac{2}{||\mathbf{\theta}||}$.
...
function hardmargin_svm(pos_data, neg_data, η=0.04, n_epochs=10000)
	...
	# Combine positive and negative data, and add bias term
    n = size(pos_data, 2)
    data = vcat(hcat(pos_data, neg_data), ones(n * 2)')'
    labels = vcat(ones(n), -ones(n))
    ...
    for epoch in 1:n_epochs
        ...
        for i in indices
            ...
            # Check if the current point violates the margin constraint
            if y_i * (dot(w, x_i)) < 1
...
function softmargin_svm(pos_data, neg_data, n_epochs=10000, C=0.12, η=0.01)
    ...
    for epoch in 1:n_epochs
        ...
        for i in indices
            ...
            # Calculate the hinge loss and slack variables
            loss = max(0, 1 - y_i * (dot(w, x_i)))

            # Calculate gradients
            grad = ifelse(loss > 0, -y_i * x_i, zeros(length(w)))

            # Update weights using the gradient and regularization
            w -= η * (grad + C * w / length(labels))
        end
    end
...
function dualsvm_smo_hard(pos_data, neg_data, n_epochs=100, λₜₒₗ=0.0001, errₜₒₗ=0.0001)
	...
	# Step 3: Main Loop (Sequential Minimal Optimization)
    for epoch in 1:n_epochs
        for i in 1:n
            Ei = sum(λ .* Y .* [K(X[j], X[i]) for j in 1:n]) + b - Y[i]

            # Check KKT conditions
            if (Y[i] * Ei < -errₜₒₗ && λ[i] < λₜₒₗ) || (Y[i] * Ei > errₜₒₗ && λ[i] > 0)
                # Randomly select a second data point
                j = rand(1:n)
                while j == i
                    j = rand(1:n)
                end

                Ej = sum(λ .* Y .* [K(X[k], X[j]) for k in 1:n]) + b - Y[j]

                # Save the old values of α1 and α2
                old_λ1, old_λ2 = λ[i], λ[j]

                # Compute L and H, the bounds on new possible α values
                if Y[i] != Y[j]
                    L = max(0, λ[j] - λ[i])
                    H = min(n, n + λ[j] - λ[i])
                else
                ...
                # Compute η (the similarity between the data points)
                ...
                # Compute the new value for α2 (unclipped)
                λ[j] -= Y[j] * (Ei - Ej) / η

                # Clip α2 to be within the bounds [L, H]
                λ[j] = max(L, min(λ[j], H))

                # Check if α2 has changed significantly
                if abs(λ[j] - old_λ2) < 1e-5
                    continue
                end

                # Compute the new value for α1
                λ[i] += Y[i] * Y[j] * (old_λ2 - λ[j])

                # Compute the bias terms b1 and b2
                b1 = b - Ei - Y[i] * (λ[i] - old_λ1) * K(X[i], X[i]) - Y[j] * (λ[j] - old_λ2) * K(X[i], X[j])
                b2 = b - Ej - Y[i] * (λ[i] - old_λ1) * K(X[i], X[j]) - Y[j] * (λ[j] - old_λ2) * K(X[j], X[j])
...
# ╔═╡ 2dab62e6-b3d7-44b7-ad33-68068ac25dc6
function eval_dualsvm(pos_data, neg_data, λ, b)
	# Step 1: Data preparation
    D = ([
        tuple.(eachcol(pos_data), 1)
        tuple.(eachcol(neg_data), -1)
	...
	for i in 1:length(Y)
        predictions[i] = sign(
            sum(λ .* Y .* [K(X[j], X[i]) for j in 1:n]) + b
        )
    end

    # Step 3: Evaluate performance
    true_positives = sum((predictions .== 1) .& (Y .== 1))
    false_positives = sum((predictions .== 1) .& (Y .== -1))
    false_negatives = sum((predictions .== -1) .& (Y .== 1))

    accuracy = (true_positives + sum(predictions .== -1)) / length(Y)
...
# ╔═╡ 7b9a60d1-83df-48d2-bc98-4c62fe9cc0cc
a, r, p, f = eval_dualsvm(points1ₜᵣₐᵢₙ, points2ₜᵣₐᵢₙ, λₕ, bₕ)
...
function dualsvm_smo_soft(pos_data, neg_data, n_epochs=100, C=1000, λₜₒₗ=0.0001, errₜₒₗ=0.0001)
    ...
    for epoch in 1:n_epochs
        for i in 1:n
            ...
            # Check KKT conditions
            if (Y[i] * Ei < -errₜₒₗ && λ[i] < C) || (Y[i] * Ei > errₜₒₗ && λ[i] > 0)
                # Randomly select a second data point
                j = rand(1:n)
                while j == i
                    j = rand(1:n)
...
# ╔═╡ 0afbe1e6-d2bf-49f7-8fa6-e1fc6e74b3a0
eval_dualsvm(sp_points1, sp_points2, λₛ, bₛ)
...
# ╔═╡ d97fef63-bef5-4efa-ad10-a993226dc0f4
begin
...
# ╔═╡ fed51097-a3fd-47e4-b7f8-5888d89f254e
md"""
!!! comment
	One-vs-Rest (OvR), also known as One-vs-All (OvA), is a strategy used in machine learning to extend binary classification algorithms to handle multiclass classification problems. In a multiclass classification problem, there are more than two classes, and the goal is to assign each instance to one of these multiple classes.

	- In the case of the MNIST dataset, where you have multiple classes (digits 0 through 9), OvR involves training a separate binary classifier for each class. Each classifier is trained to distinguish between instances of its assigned class and all other classes combined. In essence, you create 10 binary classifiers (one for each digit) to solve the multiclass problem.

	- The multi_class="ovr" setting in the logistic regression model indicates that you want to use the OvR strategy. This strategy simplifies the problem of training a multiclass classifier by reducing it to multiple binary classification subproblems.

	`LogisticRegression(multi_class="ovr")` simplifies the implementation of a multiclass logistic regression model by using the OvR strategy, making it easier to use binary classification algorithms for multiclass problems.
"""

# ╔═╡ 2f4215b0-dbda-46ee-84f1-07d637761883
begin
	print(1)
	model = LogisticRegression(multi_class="ovr")
	print(2)
	# Reshape the training data
	train_x_reshaped = reshape(train_x, 28^2, NUMBER_TRAIN_SAMPLES)'
	# Train the model
	print(3)
	fit!(model, train_x_reshaped, train_y)
	
	# Reshape the test data
	test_x_reshaped = reshape(test_x, 28^2, NUMBER_TEST_SAMPLES)'
	# Make predictions on the test set
	predictions = predict(model, test_x_reshaped)
end
...
# ╔═╡ aee9f4b4-0943-4d08-9f98-6710cbddc9fb
begin
...
# ╔═╡ 6a0b3b9a-7060-4b2d-977a-53f48d00af47
begin
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "16763d5fdb575847563d4886940c4cda196adf4e"
