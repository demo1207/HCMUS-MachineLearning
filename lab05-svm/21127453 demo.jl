...
using Plots, Distributions, LinearAlgebra, Random, ScikitLearn, PyCall
...
# ╔═╡ 9c755ff3-1d26-41e5-b55d-8b5c10efee85
@sk_import linear_model: LogisticRegression

# ╔═╡ f6eb056b-3e7c-4f82-a075-151241ff75bb
...
# ╔═╡ 8d02f9f7-de20-4b04-8ce9-e69bc6faf993
"""
...
function pla(pos_data::Matrix{Float64}, neg_data::Matrix{Float64}, 
    ...
    θ = zeros(n_features + 1, 1)

    # Concatenate bias term to input data
    pos_data = vcat(pos_data, ones(1, size(pos_data, 2)))
    neg_data = vcat(neg_data, ones(1, size(neg_data, 2)))

    # Concatenate positive and negative data
    X = hcat(pos_data, neg_data)

    # Labels for positive and negative data
    y_pos = ones(size(pos_data, 2))
    y_neg = -ones(size(neg_data, 2))
    y = vcat(y_pos, y_neg)
    ...
    for epoch in 1:n_epochs
        ...
        # Break if all examples are correctly classified
        if !misclassified
            break
        end
    end
...
# ╔═╡ 07d6d94a-ba40-4f5f-86a5-4c287638efe0
"""
...
function hardmargin_svm(pos_data, neg_data, η=0.04, n_epochs=10000)
    ...
    n_pos = size(pos_data, 2)
    n_neg = size(neg_data, 2)
    n_features = size(pos_data, 1)

    w = randn(n_features)  # Initialize weight vector randomly
...
function softmargin_svm(pos_data, neg_data, n_epochs=10000, C=0.12, η=0.01)
    ...
    for epoch in 1:n_epochs
        ...
        # Compute gradient and loss for each data point
        for i in 1:length(y)
            xi = X[:, i]
            yi = y[i]

            # Slack variable
            ζ = max(0, 1 - yi * (dot(w, xi) + b))

            # Update gradients
            gradient_w += -yi * xi + C * sign(ζ) * xi
            gradient_b += -yi + C * sign(ζ)
        ...
        # Break if loss is sufficiently small
        if loss < 1e-5
            break
        end
    end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "211fba6ebdaed6e2438c912917c21d3b85a5e588"
